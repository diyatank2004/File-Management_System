    ...
    return (
        ...
        >
            <Stack direction="row" justifyContent="space-between" alignItems="flex-start" mb={2}>
                <Box sx={{ bgcolor: '#F8FAFC', p: 1.5, borderRadius: 3, display: 'flex', border: '1px solid #F1F5F9' }}>
            ...
            <Box mb={2}>
                <Typography variant="subtitle1" fontWeight={800} noWrap title={file.filename}>
                    {highlightMatch(file.filename, query)}
                </Typography>
                <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 0.2 }}>
                    {file.relativePath || "Root Directory"} • {file.createdAt ? new Date(file.createdAt).toLocaleDateString() : "Recent"}
                </Typography>
            </Box>

            <Box sx={{ flex: 1, mb: 2, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', minHeight: '60px' }}>
                <Typography variant="body2" sx={{ color: '#475569', lineHeight: 1.5, fontSize: '0.85rem' }}>
                    {file.snippet ? highlightMatch(file.snippet, query) : "No snippet available."}
                </Typography>
            </Box>

            <Stack direction="row" justifyContent="space-between" alignItems="center" mt="auto" pt={2} borderTop="1px solid #F1F5F9">
                <Chip label={file.fileType || 'file'} size="small" sx={{ fontWeight: 900, textTransform: 'uppercase', fontSize: '0.65rem' }} />
                <Stack direction="row" spacing={1} alignItems="center">
                    <Typography variant="caption" fontWeight={700} color="text.secondary">
                        {file.size ? (file.size / 1024).toFixed(1) + " KB" : ""}
                    </Typography>
                    <Tooltip title="Delete">
                        <IconButton size="small" color="error" onClick={() => onDelete(file._id || file.id)}>
